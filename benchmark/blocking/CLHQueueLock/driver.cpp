...
void thread_f(int tid, int nr_threads, int nr_iter, int nr_exec, bool sc_runs, 
  ...
  for (int run = 0; run <= nr_threads - nr_thrds_lower_bound; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      barrier_wait(tid);
      if (tid == 0) {
        init_per_exec_global();
      }
      init_per_exec_per_thread();
      barrier_wait(tid);
      if (!sc_runs || tid <= run) {
        //
        ...
        clhqueue_sync(iter_per_thread, ui, uo, useful_in, useful_out);
...
int main(int argc, char *argv[]) {
  ...
  //
  cxxopts::Options options("clhqueue_lock_{sc,ar}", "A queue-based FIFO lock");
  options
    .add_options()
  ...
  //
  // Initialize CLHQueue mutual exclusion lock:
  //
  init_clhqueue_lock();
