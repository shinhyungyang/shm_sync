  ...
  for (int run = 0; run <= nr_threads - nr_thrds_lower_bound; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      ...
      if (!sc_runs || tid <= run) {
        ...
        cpp11_mutex_sync(iter_per_thread, ui, uo, useful_in, useful_out);
...
int main(int argc, char *argv[]) {
  ...
  //
  cxxopts::Options options("cpp11_mutex_lock","C++11 Mutex-based lock");
  options
    .add_options()
  ...
  //
  // Initialize C++11 Mutex mutual exclusion lock:
  //
  init_cpp11_mutex_lock();
