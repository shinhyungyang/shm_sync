  ...
  for (int run = 0; run <= nr_threads - nr_thrds_lower_bound; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      ...
      if (!sc_runs || tid <= run) {
        ...
        tatas_sync(iter_per_thread, ui, uo, useful_in, useful_out);
...
int main(int argc, char *argv[]) {
  ...
  //
  cxxopts::Options options("tatas_lock_{sc,ar}", "Test-And-Test-And-Set Lock");
  options
    .add_options()
  ...
  //
  // Initialize tatas mutual exclusion lock:
  //
  init_tatas_lock();
