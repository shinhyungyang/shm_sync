...
void thread_f(int tid, int nr_threads, int nr_iter, int nr_exec, bool sc_runs, 
  ...
  for (int run = 0; run <= nr_threads - nr_thrds_lower_bound; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      barrier_wait(tid);
      if (tid == 0) { init_arraylock( nr_threads ); }
      barrier_wait(tid);
      if (!sc_runs || tid <= run) {
        //
        ...
        arraylock_sync(tid, nr_threads, iter_per_thread, ui, uo, useful_in, useful_out);
...
int main(int argc, char *argv[]) {
  ...
  //
  cxxopts::Options options("arraylock_{sc,ar}", "Fair mutual exclusion algorithm.");
  options
    .add_options()
  ...
  //
  // Initialize arraylock mutual exclusion lock:
  //
  init_arraylock(nr_threads);
  ...
  auto uout_func = Decision(useful_work_ns,uout); // nano-second delay (useful_outside)
