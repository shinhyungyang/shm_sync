  ...
  for (int run = 0; run <= nr_threads - nr_thrds_lower_bound; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      ...
      if (!sc_runs || tid <= run) {
        ...
        filter_lock(tid, nr_threads, iter_per_thread, ui, uo, useful_in, useful_out);
...
int main(int argc, char *argv[]) {
  ...
  //
  cxxopts::Options options("FilterLock",
                           "Benchmark for profiling test-and-set based lock");
  options
    .add_options()
  ...
  if (nr_threads > MAX_ALLOC_NR_THREADS) { 
    std::cerr << "ERROR: requested number of threads exceeds compile-time max" << std::endl; 
    exit(EXIT_FAILURE);
  }
  ...
  for (tid = 0; tid < nr_threads; tid++) { tids[tid].join(); } 
