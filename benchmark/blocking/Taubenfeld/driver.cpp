  ...
  for (int run = 0; run <= nr_threads - nr_thrds_lower_bound; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      ...
      if (!sc_runs || tid <= run ) {
        ...
        fair_sync(tid, nr_threads, iter_per_thread, ui, uo, useful_in, useful_out);
...
int main(int argc, char* argv[]) {
  ...
  //
  cxxopts::Options options("fair_lock_{sc,ar}","Fair mutual exclusion algorithm.");
  options
    .add_options()
