...
void f_prod(int tid, int nr_pairs, int nr_iter, int nr_exec, bool sc_runs) {
  ...
  //logSimple("Producer", tid, "enters thread_f", __FILE__, __LINE__);
  ...
  for (int run = 0; run < nr_runs; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      ...
      if (!sc_runs || tid <= run ) {
        ...
        //
        //logSimple("Producer", tid, "joins run", __FILE__, __LINE__);
        ...
        //logSampleStatus("Producer", nsec_end - nsec_start,
        //                tid, exec, run, __FILE__, __LINE__);
      }
    }
  }
}
...
void f_cons(int tid, int nr_pairs, int nr_iter, int nr_exec, bool sc_runs) {
  ...
  //logSimple("Consumer", tid, "enters thread_f", __FILE__, __LINE__);
  ...
  for (int run = 0; run < nr_runs; run++) {
    ...
    for (int exec = 0; exec < nr_exec; exec++) {
      ...
      if (!sc_runs || tid - nr_pairs <= run ) {
        ...
        // Thead ``tid'' is participating in run ``run'':
        //
        //logSimple("Consumer", tid, "joins run", __FILE__, __LINE__);
        long long nsec_start = PAPI_get_real_nsec();
        ...
        //logSampleStatus("Consumer", nsec_end - nsec_start,
        //                tid, exec, run, __FILE__, __LINE__);
      }
    }
  }
...
// FIXME: re-write and move to log.cpp:
void logSampleStatus(const char * str, long long s, int tid, int exec, int run,
                     const char * file, int line)
{
